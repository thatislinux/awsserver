...
$pdf->SetFont('Arial','',12);
$pdf->Cell(80,5,'balajayaram22@gmail.com',0,1);
$pdf->Cell(189 ,5,'',0,1);//end of line
...
$header = array('Description of Goods', 'HSN/SAC', 'QTY', 'RATE($)','PER','AMOUNT($)','CGST(@9%)','SGST(@9%)','TOTAL($)');
$w = array(50, 25, 10, 18,15,21,21,21,19);
    // Header
...
$query = mysqli_query($con,"select name, product_code,qty,amount,price,tax_amount,total from sales_order where invoice='".$_GET['invoice']."'");
...
function drawColums(&$k1,&$pdf1,&$w1)
			...
			for($j=0;$j<$k;$j++)
		{
				...
				$pdf->Cell($w[0],10,'','LR');
				$pdf->Cell($w[1],10,'','LR');
				$pdf->Cell($w[2],10,'','LR',0,'R');
				$pdf->Cell($w[3],10,'','LR',0,'R');
				$pdf->Cell($w[4],10,'','LR',0,'R');
				$pdf->Cell($w[5],10,'','LR');
				$pdf->Cell($w[6],10,'','LR');
...
while($product = mysqli_fetch_array($query))
{
        ...
        $pdf->Cell($w[0],15,$i.'. '.$product['name'],'LR');
        $pdf->Cell($w[1],15,$product['product_code'],'LR');
        $pdf->Cell($w[2],15,$product['qty'],'LR',0,'R');
        $pdf->Cell($w[3],15,$product['amount'],'LR',0,'R');
		$pdf->Cell($w[4],15,'Nos','LR',0,'R');
		$pdf->Cell($w[5],15,$product['price'],'LR');
        $pdf->Cell($w[6],15,$product['tax_amount'],'LR');
        $pdf->Cell($w[7],15,$product['tax_amount'],'LR',0,'R');
		...
		$tax1_total=$tax1_total + $product['tax_amount'];
		$tax2_total=$tax2_total + $product['tax_amount'];
	...
	if($i > 9)
	{
		...
		//$pdf->Cell(170,5,$space_left,0,0,'R');
		if($space_left < 100)
		{
			$pdf->AddPage();
		}
