...
$header = array('Description of Goods', 'HSN/SAC', 'QTY', 'PRICE($)','PER','AMOUNT($)','IGST','TOTAL($)');
$w = array(51,25,14,22,15,23,25,25);
    // Header
...
while($product = mysqli_fetch_array($query))
{
        ...
        $pdf->Cell($w[6],15,($product['tax_amount']).'@'.($product['tax']).'%','LR',0,'C');
        //$pdf->Cell($w[7],15,($product['tax_amount']/2).'@'.($product['tax']/2).'%','LR',0,'C');
        $pdf->Cell($w[7],15,$product['total'],'LR',0,'R');
